  ...
  get_estimates <- function(linear_model, transf_fun = NULL, fallback = F, silent_messages = T, ...) {
    ...
    est_tibble <- model_ci %>% 
      ...
      mutate(parameter = names(model_coefs), 
             se = c(unname(model_se), ## to account for skipped model estimates
                    rep(NA, length(model_coefs) - length(model_se)))) %>% 
      select(parameter, 
             estimate, 
             se, 
             lower_ci, 
    ...
    est_tibble <- est_tibble %>% 
      mutate(p_value = c(unname(model_p),  
                         rep(NA, nrow(est_tibble) - length(model_p))), 
...
# a combi linear model function - e.g. serial linear model generation ------
  ...
  make_lm_model <- function(data = NULL, response = NULL, indep_variable = NULL, model = NULL, 
                            weight_variable = NULL, mod_fun = glm, 
